#!/usr/bin/env python3
"""
$LogFile Extractor - Extract NTFS Transaction Log from live NTFS drives
Run this script as Administrator to extract $LogFile from live NTFS system
"""
...
def extract_data_attribute(drive_handle, ntfs: NTFSBootSector, mft_record_data: bytes) -> bytes:
    ...
    if is_resident:
        ...
        while run_pos < len(data_runs):
            ...
            cluster_start = cluster_offset * ntfs.bytes_per_cluster
...
def extract_logfile(drive_letter: str, output_path: str) -> dict:
    """
    Extract $LogFile from a live NTFS drive
    $LogFile is stored in MFT record 2
    
    Args:
        drive_letter: Drive letter (e.g., 'C')
        output_path: Path to save the extracted $LogFile
    ...
    print(f"[*] Attempting to extract $LogFile from {drive_path}")
    ...
    try:
        ...
        logfile_record_num = 2
        record_offset = ntfs.mft_offset + (logfile_record_num * ntfs.mft_record_size)
        
        print(f"\n[*] Reading $LogFile (MFT Record #{logfile_record_num})")
        ...
        if not success:
            ctypes.windll.kernel32.CloseHandle(handle)
            raise IOError("Failed to read MFT record 2")
        
        print(f"[+] MFT Record 2 read ({bytes_read.value} bytes)")
        
        # Extract data attribute
        logfile_data = extract_data_attribute(handle, ntfs, mft_record_buffer.raw)
        
        ctypes.windll.kernel32.CloseHandle(handle)
        
        if not logfile_data:
            raise IOError("Could not extract $LogFile data")
        
        # Save to file
        os.makedirs(os.path.dirname(output_path), exist_ok=True)
        
        with open(output_path, 'wb') as f:
            f.write(logfile_data)
        
        print(f"\n[+] $LogFile extraction completed successfully!")
        print(f"[+] Total bytes extracted: {len(logfile_data):,}")
        ...
        return {
            ...
            "bytes_extracted": len(logfile_data),
            "file_type": "$LogFile"
        }
...
def main():
    ...
    if len(sys.argv) > 2:
        ...
        output_dir = os.path.join(os.path.dirname(__file__), "logfile_exports")
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        output_path = os.path.join(output_dir, f"$LogFile_{drive_letter.upper()}_{timestamp}.bin")
    ...
    if result["success"]:
        print("\n" + "=" * 70)
        print("[+] SUCCESS! $LogFile extracted successfully")
        print("=" * 70)
        sys.exit(0)
    else:
        print("\n" + "=" * 70)
        print("[!] FAILED! Could not extract $LogFile")
        print("=" * 70)
        sys.exit(1)
