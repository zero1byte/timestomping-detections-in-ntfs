...
def _get_actual_mft_size(data, mft_record_size, bytes_per_sector):
    ...
    Args:
        data: Raw bytes of MFT record 0
        mft_record_size: Size of one MFT record in bytes
        bytes_per_sector: Sector size (typically 512)

    Returns:
        Actual MFT size in bytes, or None if it cannot be determined
    """
    ...
    # Apply fixup array to get correct data across sector boundaries
    ...
    while attr_offset + 8 <= mft_record_size:
        ...
        if attr_type == 0x80:  # $DATA
            ...
            if non_resident:
                # Real (logical) size at +0x30, allocated size at +0x28
...
def extract_mft(drive_letter: str, output_path: str) -> dict:
    ...
    Reads the actual MFT size from record 0's $DATA attribute so that only
    the real MFT content is extracted (not a fixed 97.66 MB every time).
    ...
    try:
        ...
        if actual_mft_size and actual_mft_size > 0:
            ...
            # Fallback: use a conservative cap if record 0 couldn't be parsed
        ...
        # Re-seek to MFT start (we consumed record 0 above)
        ...
        print(f"[*] Reading MFT... (~{mft_size / (1024*1024):.2f} MB)")
